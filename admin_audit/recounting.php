        ...
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            ...
            <div class="form-group">
              ...
              <input type="number" id="recount_qty" class="form-control" disabled>
            </div>
...
$(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.valid) {
                            ...
                            let unit = response.unit ? response.unit.toUpperCase() : '';
                            let html = `
                                ...
                                <p><strong>Unit:</strong> <span class="label label-primary">${response.unit || 'PCS'}</span></p>
                            ...
                            if (unit === 'KGS' || unit === 'KSG') {
                                $('#recount_qty').attr('step', 'any').attr('placeholder', 'e.g. 5.5');
                            } else {
                                $('#recount_qty').removeAttr('step').attr('placeholder', 'e.g. 10');
                            }
                            ...
                            $('#recount_qty').removeAttr('step').attr('placeholder', '');
    ...
    $('#recount_item_code, #recount_qty').on('keypress', function(e) {
        if (e.which === 13) {
            e.preventDefault();
            if (!$('#btn_add_recount').prop('disabled') && $(this).attr('id') === 'recount_qty') {
                $('#btn_add_recount').click();
            }
        }
    });

    $('#recount_qty').on('keypress', function(e) {
        let unit = currentItem && currentItem.unit ? currentItem.unit.toUpperCase() : '';
        if (unit !== 'KGS' && unit !== 'KSG') {
            // Prevent decimal point character (46 is dot)
            if (e.which === 46 || e.key === '.') {
                e.preventDefault();
            }
        }
