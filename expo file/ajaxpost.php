...
if(isset($_POST['loaditemmaster'])){
	...
	$sqlinsert = oci_parse($conn, "INSERT INTO MASTER_ITEM (item_code,item_name,barcode,price,dept,CURR_STOCK,SHOP_CODE,CH_PI, CH_STATUS,VC_GROUP,VC_SUBGROUP)
	select item_code,item_name,barcode,price,dept,S.NU_BALANCE_QTY,S.VC_SHOP_CODE,A.CH_PI, A.CH_STATUS,GROUPS,SUB_GROUP
	from makess.vs_item_audit_final@db_link_shop a
...
if(isset($_POST['loadingdeptwise'])){
	...
	$sqlinsert = oci_parse($conn, "
		INSERT INTO MASTER_ITEM (item_code,item_name,barcode,price,dept,CURR_STOCK,SHOP_CODE,CH_PI, CH_STATUS,VC_GROUP,VC_SUBGROUP)
		SELECT * FROM 
	(select item_code,item_name,barcode,price,dept,S.NU_BALANCE_QTY,S.VC_SHOP_CODE,A.CH_PI, A.CH_STATUS,GROUPS,SUB_GROUP
		from makess.vs_item_audit_final@db_link_shop a
	 	left outer join pos.shop_stock_summary@db_link_shop s on s.vc_item_code=a.item_code and s.vc_comp_code='01'
...
if(isset($_POST['uploadnew'])){
	...
	$sqldel = oci_parse($conn, "delete from POS.TEMP_DT_STOCK_ADJUSTMENT@db_link_shop where vc_shop_code='{$sc}' ");
	oci_execute($sqldel);
	oci_free_statement($sqldel);

	
	$sql = oci_parse($conn, "SELECT VC_ITEM_CODE,VC_SHOP_CODE,VC_AUDIT_QTY FROM ZS_STOCK_AUDIT_ERP_NEW");
