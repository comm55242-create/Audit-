...
<!---------->
            ...
            <div class="btn-group pull-right">
				...
				<span> <?php echo strtoupper($_SESSION['username'])?></span> <span class='label label-danger' id='nav_pending_count'>...</span></button>
...
<?php } ?>
    ...
    $(document).ready(function() {
        $.ajax({
            url: 'ajax_get_pending_count.php',
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                if (response && response.count !== undefined) {
                    $('#nav_pending_count').text(response.count);
                } else {
                    $('#nav_pending_count').text('0');
                }
            },
            error: function() {
                $('#nav_pending_count').text('?');
            }
        });
    });
    </script>
...
<div class="list-group">
	...
	<a class="ajax-link list-group-item" href="recounting.php"> Recounting</a>
	<a class="ajax-link list-group-item" href="recount_approvals.php" style="padding-left: 30px; font-size: 0.9em; background-color: #f9f9f9;"><i class="glyphicon glyphicon-bell"></i> Mobile Approvals</a>
</div>
<?php
$oot = $_SESSION['staff_id'];
$add = oci_parse($conn, "SELECT ADMIN FROM USERS WHERE STAFF_ID = '{$oot}'");
