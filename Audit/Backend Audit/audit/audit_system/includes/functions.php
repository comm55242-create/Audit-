<?php
//This files stores all basic functions
	...
	function mysql_prep( $value ){
		...
		if($new_enough_php){ //PHP v4.3.0 or higher
			...
			// if magic quotes aren't already an then add slashes manually
			if( !$magic_quotes_active ){ $value = addslahes( $value ); }
	...
	function get_pages_for_subjects($subject_id, $public = true){
		...
		$query = "SELECT * 
				FROM articles ";
		$query .= "WHERE menu_id = {$subject_id} ";
		if ($public){
			$query .= "AND visible = 1 ";
		}
	...
	function get_subject_by_id($subject_id){
		...
		// Remember:
		// if no rows are return, fetch_array will return false
